...
interest <- AchilliesCRISPR[, -1]

GenesOfInterest <- read.table(
    "/Users/lupienlab/Desktop/lupien_lab_stanley/PCa_Project/AchillesData/GenesofInterest.txt",
    sep = "\t",
...
for (i in 1:length(GenesOfInterest)) {
    wach <- c(
        wach,
        grep(pattern = GenesOfInterest[i], x = interest[, 1])
    )
}
wach <- wach[-c(4, 6, 31:35, 43:56)]
...
TestEnrichment <- function(
    # Matrix; permutations are rows, distinct entities being tested are columns
    Permutations,
    # Vector of ncol(Permutations) where the observed value is shown
    Enrichments,
    # Names of parameters
    CellLines,
    # Colour of boxplots
    ...
    for (i in 1:ncol(MedPerm)) {
        MedPerm[, i] <- (Permutations[, i] - Means[i]) / SDs[i]
    }
    # normalize Enrichments
    for (i in 1:ncol(MedPerm)) {
        EnNorm[i] <- (EnNorm[i] - Means[i]) / SDs[i]
    }
...
pMat <- matrix(
    ncol = 2,
    nrow = DesiredNumberOfPermutations
)
...
DesiredNumberOfPermutations <- 1e4
pMat <- matrix(
    ncol = ncol(interest) - 1,
    nrow = DesiredNumberOfPermutations
)
for (i in 1:DesiredNumberOfPermutations) {
...
TestEnrichment(
    pMat[, order(Enri)],
    Enri[order(Enri)],
    colnames(interest)[2:ncol(interest)][order(Enri)],
    c(
        "red",
        "blue",
        "darkorchid3",
